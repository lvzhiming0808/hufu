    ...
    <div class="mark-box">
      <div class="box">
        <div class="lefts">
            ...
            <h3>备注</h3>
            <el-input type="textarea" v-model="desc" placeholder="请输入250字以内的备注哦！" class="approval-area-input" @keyup.native="codeLen"></el-input>
            <p class="tips"><span v-if="isTips">不能超过250字哦！</span></p>
  ...
  methods: {
    ...
    goDetail(row, event) {
      if (event.target.tagName.toLowerCase() === 'div') {
        ...
        this.$router.push(`/audioPlay?showDetail=${row.id}&showShare=true`)
      }
    },
    dialogVisible(id) {
      this.isMark = true
    ...
    Sure() {
      if (this.input2.trim().length > 0) {
        if (this.desc.length > 0) {
          this.isMark = false
          fetch({
            url: `/boutiques/records/share?organizationIds=${this.branch.toString()}&recordId=${this.ids}&content=${this.desc}`,
            method: 'post'
          })
          .then(res => {
            if (res === true) {
